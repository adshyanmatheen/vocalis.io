...
export const AudioPlayerElement = (props: AudioPlayerElementProps) => {
  ...
  const { data: _data, ...validProps } = props as Record<string, unknown>

  return (
    // oxlint-disable-next-line eslint-plugin-jsx-a11y(media-has-caption) -- audio player captions are provided by consumer
    <audio data-slot="audio-player-element" slot="media" src={src} {...validProps} />
  )
}
...
export const AudioPlayerControlBar = ({
  ...
  <MediaControlBar
    ...
    className={cn('flex w-full h-10 items-center justify-center py-0', className)}
    {...props}
  >
    <ButtonGroup
      orientation="horizontal"
      className="mx-auto flex w-full h-10 items-center justify-center gap-2"
    >
      {children}
    </ButtonGroup>
  </MediaControlBar>
...
export const AudioPlayerPlayButton = ({ className, ...props }: AudioPlayerPlayButtonProps) => (
  <Button
    ...
    variant="ghost"
    nativeButton={false}
    className={cn('h-8 w-8 min-w-0 rounded-none p-0', className)}
    render={
      <MediaPlayButton
        className={cn('h-8 w-8 min-w-0 bg-transparent border-0 rounded-none p-0', className)}
